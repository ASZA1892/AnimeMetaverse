...
-- Robust data-driven server-side combat handler with Guard/Parry/Dash
...
CombatRemote.OnServerEvent:Connect(function(player, action, data)
    ...
    if type(finalDamage) ~= "number" then
        warn(("DEBUG: finalDamage is not a number. Treating as 0."):format(tostring(finalDamage)))
        finalDamage = 0
    end
...
dbg("✅ CombatHandler server initialized (with Guard/Parry/Dash)")