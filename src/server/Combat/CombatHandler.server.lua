...
local function canProcessAttack(player)
	...
	if now - last < ATTACK_MIN_INTERVAL then
		return false
	end
...
local function applyEffects(targetModel, move, attacker)
	...
	if move.launch then
		...
		if root then
			root.AssemblyLinearVelocity = Vector3.new(0, move.launchPower or 25, 0)
		end
	end
	...
	if move.stun and move.stun > 0 then
		...
		if targetPlayer and CombatStateMachine and CombatStateMachine.ForceState then
			CombatStateMachine.ForceState(targetPlayer, "Disabled", {
...
CombatRemote.OnServerEvent:Connect(function(player, action, data)
	...
	-- Dash
	if action == CombatActions.ClientToServer.DASH then
		if DashHandler then
			DashHandler.ProcessDash(player, data, CombatStateMachine, CombatRemote)
		end
		return
	end

	-- Substitution
	if action == CombatActions.ClientToServer.SUBSTITUTION then
		if SubstitutionHandler then
			SubstitutionHandler.ProcessSubstitution(player, data, CombatStateMachine, CombatRemote)
		end
		return
	end

	-- Grapple
	if action == CombatActions.ClientToServer.GRAB_ATTEMPT then
		if GrappleHandler then
			GrappleHandler.ProcessGrapple(player, data, CombatStateMachine, CombatRemote)
		end
		return
	end

	if action == CombatActions.ClientToServer.GRAB_RELEASE then
		if GrappleHandler then
			GrappleHandler.ProcessGrappleRelease(player, data, CombatStateMachine, CombatRemote)
		end
		return
	end

	-- Attack
	if action ~= CombatActions.ClientToServer.M1_ATTACK then
		dbg("Unknown action:", tostring(action))
		return
	end
	...
	if not canProcessAttack(player) then
		dbg("Attack rate limited for", player.Name)
		return
	end
	...
	if move.requiresDash and currentState ~= "Dashing" then
	...
	local hitboxSize = move.hitboxSize or Vector3.new(4, 4, 4)
	...
	if not wasParried then
		...
		if not hitPlayer or CombatStateMachine.GetState(hitPlayer) ~= "Blocking" then
			applyEffects(hitModel, move, player)
		end
	end

	if move.lunge then
		rootPart.AssemblyLinearVelocity = forward * (move.lunge * 20)
	end
