...
-- With Guard/Parry support and client-side animation anticipation
...
-- AnimationLoader: client-side animation helper (must expose playAttackAnimation)
local function safeRequireCandidates(candidates)
    for _, inst in ipairs(candidates) do
        if inst and typeof(inst) == "Instance" then
            local className = inst.ClassName or "nil"
            print(("DEBUG: trying require candidate -> ClassName=%s Name=%s Parent=%s"):format(className, tostring(inst.Name), tostring(inst.Parent and inst.Parent.Name)))
            local ok, mod = pcall(require, inst)
            if ok and mod then
                print(("DEBUG: required module from %s"):format(tostring(inst:GetFullName())))
                return mod
            else
                warn(("safeRequireCandidates: require failed for %s -> %s"):format(tostring(inst:GetFullName()), tostring(mod)))
            end
        end
    end
    return nil
...
do
    ...
    -- 1) ReplicatedStorage.Shared.Animations.AnimationLoader
    local shared = ReplicatedStorage:FindFirstChild("Shared")
    if shared then
        local animFolder = shared:FindFirstChild("Animations")
        if animFolder then
            local candidate = animFolder:FindFirstChild("AnimationLoader")
            if candidate then table.insert(candidates, candidate) end
        end
    end

    -- 2) ReplicatedStorage.Animations.AnimationLoader
    local altAnimFolder = ReplicatedStorage:FindFirstChild("Animations")
    if altAnimFolder then
        local candidate = altAnimFolder:FindFirstChild("AnimationLoader")
        if candidate then table.insert(candidates, candidate) end
    end

    -- 3) StarterPlayerScripts/Animations/AnimationLoader (local fallback)
    do
    ...
    -- 4) Search anywhere under ReplicatedStorage for a module named AnimationLoader
    do
    ...
    if not AnimationLoader then
        warn("CombatController: AnimationLoader not found in any candidate locations; client will use fallback behavior.")
        AnimationLoader = nil
    end
end
...
-- Fire attack to server (plays animation locally first)
local function fireAttack()
    local moveId = findMoveForCurrentInput()
    if moveId then
        -- Play animation immediately on client for instant feedback
